/**
 * readInputForm parses HTML elements into CONFIG object
...
export function readInputForm() {
    ...
    for (let i = 1; i < NUMBER3DVIEWS + 1; i++) {
        ...
        tempViewSetup["systemDimension"] = {
            "x": Number(tempFormResult["view" + i + "XDim"]),
            "y": Number(tempFormResult["view" + i + "YDim"]),
            "z": Number(tempFormResult["view" + i + "ZDim"])
        };
        const tempU = {
            "u11": Number(tempFormResult["view" + i + "LatVec11"]),
        ...
        if (boolSpatiallyResolved) {
            ...
            tempViewSetup["spatiallyResolvedData"]["numGridPoints"] = {
                "x": Number(tempFormResult["view" + i + "XNumPoints"]),
                "y": Number(tempFormResult["view" + i + "YNumPoints"]),
                "z": Number(tempFormResult["view" + i + "ZNumPoints"])
            };
            tempViewSetup["spatiallyResolvedData"]["gridSpacing"] = {
                "x": Number(tempFormResult["view" + i + "XSpacing"]),
...
/**
 * normalizes lattice array in parsing form
 * @param  {vector array} U
 */
