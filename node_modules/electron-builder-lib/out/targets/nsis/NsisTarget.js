...
class NsisTarget extends (_core || _load_core()).Target {
    constructor(packager, outDir, targetName, packageHelper) {
        ...
        this.options = targetName === "portable" ? Object.create(null) : Object.assign({}, this.packager.config.nsis, { preCompressedFileExtensions: [".avi", ".mov", ".m4v", ".mp4", ".m4p", ".qt", ".mkv", ".webm", ".vmdk"] });
        if (targetName !== "nsis") {
    ...
    getPreCompressedFileExtensions() {
        ...
        return result == null ? null : (0, (_builderUtil || _load_builderUtil()).asArray)(result).map(it => it.startsWith(".") ? it : `.${it}`);
    }
    /** @private */
    buildAppPackage(appOutDir, arch) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            const archiveOptions = {
                ...
                excluded: preCompressedFileExtensions == null ? null : preCompressedFileExtensions.map(function (it) {
                    return `*${it}`;
                })
            };
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            if (preCompressedFileExtensions != null) {
                for (const [arch, dir] of _this8.archs.entries()) {
                    const preCompressedAssets = yield (0, (_fs || _load_fs()).walk)(_path.join(dir, "resources"), function (file, stat) {
                        return stat.isDirectory() || preCompressedFileExtensions.some(function (it) {
                            return file.endsWith(it);
                        });
                    });
                    if (preCompressedAssets.length !== 0) {
                        const macro = new (_nsisScriptGenerator || _load_nsisScriptGenerator()).NsisScriptGenerator();
                        for (const file of preCompressedAssets) {
                            macro.file(`$INSTDIR\\${_path.relative(dir, file).replace(/\//g, "\\")}`, file);
                        }
                        scriptGenerator.macro(`customFiles_${(_builderUtil || _load_builderUtil()).Arch[arch]}`, macro);
                    }
                }
            }
